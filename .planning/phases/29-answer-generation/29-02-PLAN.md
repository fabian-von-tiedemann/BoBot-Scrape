...
<task type="auto">
  ...
  <action>
    ...
    ANSWER_GENERATION_PROMPT constant (EXTRACTION-STYLE - quote source text):
    - PREFER quoting relevant text directly from sources
    - Paraphrase ONLY when direct quotes would be awkward or excessively long
    - Always include direct quotes for key facts, procedures, and specific instructions
    - Klarsprak requirements: max 15 words per sentence, active voice ("Du tvattar" not "ska tvattas"), use "du"
    - Citation format: [source:document.md#section] inline immediately after quoted/referenced content
    - Length: 1-4 sentences (can be slightly longer to include key quotes)
    - Handle partial coverage: note what's not covered in sources
  ...
  <done>
    ...
    - ANSWER_GENERATION_PROMPT enforces extraction-style (quoting) and klarsprak
...
<task type="auto">
  ...
  <action>
    ...
    4. Spot-check quality (EXTRACTION-STYLE verification):
       - Answers are in Swedish
       - Citations reference real documents from converted/
       - Answers contain direct quotes from source documents (not just paraphrases)
       - Key facts and procedures are quoted verbatim
  ...
  <done>
    ...
    - Answers are grounded in source documents with direct quotes
...
- Answers use extraction-style: preferring direct quotes over paraphrasing (AGEN-04)
